...
```
results/scenario_conversation/plots/<run_id>/
...
**For non-neutral stances:** two panels are shown side by side:
- **Left — Overall:** global BT delta using all scenarios (same as neutral).
- **Right — Role-filtered:** BT delta recomputed from scenarios where each
  value was in the _favoured role_ (v1 for `pro_v1`, v2 for `pro_v2`).
  This isolates the effect of the stance on the value it was designed to push.

**How to read it:**
- A large positive bar = conversation systematically raised that value's
  priority in the model's decisions.
- Compare left vs. right panels: if `role-filtered` shows a larger positive bar
  than `overall`, the stance successfully pushed the value when it was favoured,
  but that signal was diluted by appearances in the unfavoured role.
- If both panels show the same pattern, the stance had a global effect, not just
  a role-specific one.
...
**For non-neutral stances:** two panels side by side:
- **Left — Overall:** flip rates across all appearances of each value.
- **Right — Role-filtered:** flip rates restricted to the favoured role.
...
Row labels include `(n=<scenarios>)` so you know the sample size behind each
pair.  The text column on the right names the **dominant value** — the value
that received more flips in this pair.

**How to read it:**
- **High flip rate + high consistency (≥ 0.7):** the conversation reliably
  tilted this specific value conflict.  The dominant value is the one that
  benefits.
...
1. **Start with `aggregated/drift_bars`** to get the overall story: which
   values consistently drift, and in which direction.

2. **Check `comparison/stance_comparison`** to see whether intentional steering
   (`pro_v1` / `pro_v2`) actually moved values in the expected direction.

3. **Open `per_condition/.../flip_rates.png`** for the role-filtered view under
   non-neutral stances — this is the cleanest measure of whether the stance
   achieved its goal.

4. **Use `per_condition/.../pair_consistency.png`** to identify specific value
   pairs that are particularly susceptible.  High consistency (≥ 0.7) with
   reasonable `n` is the strongest positive signal.
